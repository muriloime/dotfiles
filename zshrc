...
function read_loud() {
    ...
    # Check dependencies (only check once to keep it fast)
    if ! command -v pipx &> /dev/null; then echo "Error: pipx is required."; return 1; fi
    ...
    # Try speaking. If successful (exit code 0), return. 
    # We suppress output to keep terminal clean, but let errors show if debug needed.
    ...
    local model_file="en_US-lessac-medium.onnx"
    ...
    # Try piper. 
    # Using 'aplay' which is standard on Linux. 
    # If edge-tts failed (no internet), this runs instantly.
...
# Optional: Set as your 'standard' terminal TTS
alias say="read_loud"
alias spd-say="read_loud"
